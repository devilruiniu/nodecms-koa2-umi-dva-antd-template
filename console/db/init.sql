...
CREATE DATABASE /*!32312 IF NOT EXISTS*/`db_admin-template` /*!40100 DEFAULT CHARACTER SET utf8 */;

USE `db_admin-template`;

/* 创建账户组表`t_bas_account_group`,存在的话先删除 */

DROP TABLE IF EXISTS `t_bas_account_group`;

CREATE TABLE `t_bas_account_group` (
  `id` bigint(20) NOT NULL AUTO_INCREMENT COMMENT '账户组id',
  `menu_ids` varchar(255) DEFAULT NULL COMMENT '菜单id，以逗号分隔的字符串,默认为NULL,表示包含所有菜单',
  `name` varchar(128) NOT NULL COMMENT '账户组中文名称',
  `description` varchar(255) NOT NULL COMMENT '账户组中文描述',
  `status` tinyint(1) NOT NULL DEFAULT 1 COMMENT '账户组状态，默认1（1：启用，2：禁用）',
...
/* 创建账户表`t_bas_account`,存在的话先删除 */

DROP TABLE IF EXISTS `t_bas_account`;

CREATE TABLE `t_bas_account` (
  `id` bigint(20) NOT NULL AUTO_INCREMENT COMMENT '账户id',
  `account_group_id` bigint(20) NOT NULL DEFAULT 1 COMMENT '账户组id 默认为1（超级管理员）',
  `account` varchar(128) NOT NULL COMMENT '账户',
  `password` varchar(128) NOT NULL COMMENT '密码',
  `name` varchar(128) NOT NULL COMMENT '账户角色中文名称（如：管理员）',
...
CREATE TABLE `t_bas_menu` (
  ...
  `parent_id` bigint(20) NOT NULL DEFAULT 0 COMMENT '父级菜单ID，默认0 （0：没有父级ID）',
  `name` varchar(256) NOT NULL COMMENT '菜单中文名称',
  `path` varchar(128) NOT NULL COMMENT '该菜单对应的项目路径（含父路径）的ID值，以|隔开',
  `route` varchar(128) DEFAULT NULL COMMENT '该菜单对应的route值',
  `icon` varchar(256) DEFAULT NULL COMMENT '菜单对应的图标',
  `level` bigint(20) NOT NULL DEFAULT 1 COMMENT '当前层级，默认1 （1：第一层级）',
  `is_nav_menu` tinyint(1) NOT NULL DEFAULT 1 COMMENT '是否为导航菜单，默认1（1：是，2：否）',
  `status` tinyint(1) NOT NULL DEFAULT 1 COMMENT '账户组状态，默认1（1：启用，2：禁用）',
  `ext1` varchar(256) DEFAULT NULL COMMENT '扩展字段2',
  `ext2` varchar(256) DEFAULT NULL COMMENT '扩展字段3',
  `ext3` varchar(256) DEFAULT NULL COMMENT '扩展字段4',
  `meta_created` datetime NOT NULL DEFAULT current_timestamp() COMMENT '创建时间',
  `meta_modified` datetime NOT NULL DEFAULT current_timestamp() COMMENT '最后一次修改时间',
  `meta_logic_flag` tinyint(1) NOT NULL DEFAULT 2 COMMENT '逻辑删除标识，默认2（1：是，2：否）',
  PRIMARY KEY (`id`)
...
(1,"账号组管理","1|2","/sys/accountgroup",NULL,2,1),
(1,"账号管理","1|3","/sys/account",NULL,2,1),
(1,"菜单管理","1|4","/sys/menu",NULL,2,1);
