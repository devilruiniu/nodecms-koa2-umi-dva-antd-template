...
class Index extends React.Component {
  ...
  componentDidMount() {
    // 获取账号组信息
    this.props.dispatch({
      type: `${namespace}/getAccountGroupList`,
    });
  }
  ...
  onDeleteItem = (id) => {
    ...
    const { list, pagination } = account;
    this.props.dispatch({
      type: `${namespace}/deleteAccount`,
      payload: { id },
    }).then(() => {
      this.handleRefresh({
        pageIndex:
  ...
  onEditItem = (item) => {
    ...
    item.account_group_id = item.account_group_id.toString();
    this.props.dispatch({
  ...
  render() {
    const { location, account, loading, dispatch } = this.props;
    const { query } = location;
    const { list, pagination, modalVisible, modalType, currentItem, accountGroupList } = account;
    // 查询组件属性
    const filterProps = {
      filter: {
        ...query,
    ...
    const tableProps = {
      list,
      pagination,
      loading: loading.effects[`${namespace}/getAccountList`],
      columns: columns,
      /**
       * 表格翻页事件
       * @param page
    ...
    // 新增/编辑弹框确定按钮触发接口事件
    const confirmDispatch = modalType === 'create' ? `${namespace}/addAccount` : `${namespace}/editAccount`;
    // 新增/编辑弹框配置属性
    const modalProps = {
      item: currentItem,
      modalType,
      ...
      title: `${modalType === 'create' ? '创建账户' : '编辑账户'}`,
      centered: true,
      onOk: (data) => {
        dispatch({
